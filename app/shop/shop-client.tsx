...
//       ...bundleKits.map((b) => b.categoryName || "Bundle Kits"),
//     ])
//   ).sort();
...
//     const priceMatch =
//       offer.discountedPrice >= priceRange[0] &&
//       offer.discountedPrice <= priceRange[1];

//     return categoryMatch && priceMatch;
//   });

//   const filteredBundleKits = bundleKits.filter(
//     (kit) => kit.price >= priceRange[0] && kit.price <= priceRange[1]
//   );
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        const [offersRes, bundleKitsRes] = await Promise.all([
          fetch("/api/offers"),
          fetch("/api/bundle-kits"),
        ]);
        ...
        const [offersData, bundleKitsData] = await Promise.all([
          offersRes.json(),
          bundleKitsRes.json(),
        ]);
