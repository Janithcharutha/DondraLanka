...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title =
    offer.productName || offer.product?.name || offer.title || "Special Offer";
  ...
  const description =
    ...
    `Explore this special seafood offer from DONDRA LANKA.`;

  const resolvedSlug =
    offer.slug || offer.productSlug || offer.product?.slug || slug;

  return buildMetadata({
    title,
    description,
    path: `/offers/${resolvedSlug}`,
    image,
    keywords: [title, "special seafood offer", "dried fish promotion"],
  });
}
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: "Offers",
        item: "https://www.dondralanka.com/offers",
      },
      {
        "@type": "ListItem",
        position: 3,
        name: title,
        item: `https://www.dondralanka.com/offers/${resolvedSlug}`,
      },
    ],
  };
